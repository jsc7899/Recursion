 ...
 */
int findp(int cents, int p) {
	if(cents >= 1) {
		p++;
		return findp(cents - 1, p);
	}
	return p;
}
int findn(int cents, int n) {
	if(cents >= 5 && cents % 5 == 0) {
		n = cents/5;
		return n;
	}
	if(cents >= 5) {
		n++;
		return findn(cents - 5, n);
	}
	return n;
}
int findd(int cents, int d) {
	if(cents >= 12 && cents % 12 == 0) {
		d = cents/12;
		return d;
	}
	if(cents >= 12) {
		d++;
		return findd(cents-12, d);
	}
	return d;
}
Martian change(int cents) {
	...
	d1 = findd(cents,d1);
	int n1 = 0;
	n1 = findn(cents-(12*d1),n1);
	int p1 =  0;
	p1 = findp(cents-(12*d1)-(5*n1),p1);
	martian1.pennies = p1;
	martian1.nicks =  n1;
	martian1.dodeks = d1;
	...
	n2 = findn(cents,n2);
	int d2 = 0;
	d2 = findd(cents-(5*n2),d2);
	int p2 =  0;
	p2 = findp(cents-(12*d2)-(5*n2),p2);
	martian2.pennies = p2;
	martian2.nicks =  n2;
	martian2.dodeks = d2;
	if(p1+n1+d1 < p2+n2+d2) return martian1;
	else return martian2;
}
...
int findnv(int cents, int n, int val) {
	if(cents >= val && cents % val == 0) {
		n = cents/val;
		return n;
	}
	if(cents >= val) {
		n++;
		return findn(cents - val, n);
	}
	return n;
}
int finddv(int cents, int d, int val) {
	if(cents >= val && cents % val == 0) {
		d = cents/val;
		return d;
	}
...
Martian change(int cents, int nick_val, int dodek_val) {
	Martian martian1 = {0};
	int d1 = 0;
	d1 = finddv(cents,d1, dodek_val);
	int n1 = 0;
	n1 = findnv(cents-(dodek_val*d1),n1, nick_val);
	int p1 =  0;
	p1 = findpv(cents-(d1*dodek_val)-(n1*nick_val),p1);
	martian1.pennies = p1;
	martian1.nicks =  n1;
	martian1.dodeks = d1;
	...
	Martian martian2 = {0};
	int n2 = 0;
	n2 = findnv(cents,n2, nick_val);
	int d2 = 0;
	d2 = finddv(cents-(nick_val*n2),d2, dodek_val);
	int p2 =  0;
	p2 = findpv(cents-(dodek_val*d2)-(nick_val*n2),p2);
	martian2.pennies = p2;
	martian2.nicks =  n2;
	martian2.dodeks = d2;
	if(p1+n1+d1 < p2+n2+d2) return martian1;
